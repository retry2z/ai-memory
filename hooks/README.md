# memorize Hooks — Auto-Save for Terminal AI Tools

These hook scripts make memorize save automatically. No manual "save" commands needed.
...
- **`STATE_DIR`** — Where hook state is stored (defaults to `~/.memorize/hook_state/`)
- **`MEMPAL_DIR`** — Optional. Set to a conversations directory to auto-run `memorize mine <dir>` on each save trigger. Leave blank (default) to let the AI handle saving via the block reason message.
...
```bash
memorize mine <dir>               # Mine all files in a directory
memorize mine <dir> --mode convos # Mine conversation transcripts only
```
...
Check the hook log:
```bash
cat ~/.memorize/hook_state/hook.log
```
